...
#define GWS_DEF NULL
  ...
  typedef struct
  {            /* coordinate rectangle pointer */
    Gpoint ul; /* upper left point */
    Gpoint lr; /* lower right point */
  } Grect;

  typedef struct
  {              /* dimension in integer values */
  ...
  DLLEXPORT void gks_inq_color_rep(int wkid, int index, int type, int *errind, double *red, double *green,
                                   double *blue);
